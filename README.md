...
不管以上哪种方式，都**要求上一次变更没有完成之前，不能进行下一次变更**。

> 直接切换出现异常例子
![脑裂](https://user-images.githubusercontent.com/83362909/203730512-dcb36dfe-b3ea-419b-88f5-419d13cb2c70.png)
- `Server4` 和 `Server5` 作为新节点加入集群，请求通知给 `Server3` (`Leader`)；
- `Leader` 给它们同步日志，然后尝试将它们封装为 `MemberChange` 消息传播给其他节点前，发生了宕机；
- **此时集群中出现两个集群视图，一个是以 `Server1` 、`Server2`、`Server3` 为集群，一个以 `Server1` 、`Server2`、 `Server3`、`Server4`、`Server5` 为集群**；
- 在 `Server3` 宕机后，集群节点超时选举；
- **由于此时集群中节点的日志都是一样的，但是视图不同，并且不同的视图都构成了其视图"大多数"原则，就可能会出现"脑裂"问题**；
  -  `Server1` 得到 `Server2` 的投票，`2/3` 构成大多数；
  -  `Server4` 得到 `Server3`、`Server5` 的投票，`3/5` 构成大多数； 
### 单节点变更算法
...
这种方式毫无疑问是安全的，因为集群中即使出现两个集群视图，由于双方只比另一个视图差异一个节点，。
...
> 日志覆盖问题
![覆盖](https://user-images.githubusercontent.com/83362909/203730649-6ad378c9-086e-4995-96b2-6c122e53c1e3.png)
新增 `u` 和 `v` 节点：

- 在没有完成 `u` 成员添加的情况下(只同步了不到一半节点)，`a`  作为 `Leader` 宕机；
  - 只有 `a` 和 `u` 知道 `u` 加入；

- 重新选出 `d` 为 `Leader` 进行下一次 `v` 成员添加，同步到一半节点；
  - 只有 `c`、`d`、`v` 知道 `v` 存在；
- 此时 `d` 宕机，而  `a` 恢复，**`a` 由于 `u` 和 `b` 投票，并且不知道集群多了 `v`** ，所以自认为成为 `Leader`，**而 `Cv` 是提交了的，如果此时 `a` 再提交日志就会覆盖掉 `Cv`**；

> 解决办法

**在 `Leader` 进行单节点成员变更之前，进行一次 `no-op` 日志提交，完成后才进行成员变更**。

这样就可以**保证日志落后的节点一定不会成为 `Leader`** ，上述例子，`L1` 在 `Cv` 前同步 `no-op` 日志，保证 `a` 一定不会成为 `Leader`.
...
- `Follower` 收到 `C-old,new` 后，立刻应用到 `Follower` 的集群成员中
  - 此时集群就进入到**联合一致**【每次追加日志都需要新旧配置节点一半以上才算成功】的状态，**任何读写操作(包括选举)都需要经过新节点集群和旧节点集群的一半以上**，才算成功
